...
function DashExam() {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/exams/course/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/add/exams', questionData);
      setQuestion({
        questionText: '',
        options: ['', '', '', ''],
        correctOption: 0,
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/delete/exams/${id}`);
      fetchQuestions();
    } catch (error) {
      console.error(error);
    }
  ...
  const handleUpdate = async () => {
    try {
      ...
      await axios.put(`http://localhost:5000/update/exams/${question._id}`, questionData);
      ...
      <div className="main-dash">
        ...
        <div className="dashboard p-4 space-y-4">
          ...
          <div className="question-form border p-4">
            <h2 className="text-xl font-semibold mb-4">Create and Update Questions Exam</h2>
